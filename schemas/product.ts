...
export const ProductSchema = z.object({
  name: z.string().min(1).max(200).trim().describe('Nombre comercial del producto.'),
  description: z
    .string()
    .max(5000)
    .trim()
  ...
  category: z.string().max(100).trim().optional().describe('Categoría o rubro para filtros.'),
  price: z.number().nonnegative().max(99_999_999).describe('Precio de lista en moneda base.'),
  image_url: z
    .string()
    .url()
    .nullable()
  ...
  active: z.boolean().default(true).describe('Si el producto se muestra en catálogos activos.'),
  attributes: z
