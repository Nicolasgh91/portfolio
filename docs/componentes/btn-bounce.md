...
- Un hijo **`span.arrow`** (o contenedor equivalente) con **`aria-hidden="true"`** envuelve la flecha decorativa (`→` o `<svg>`). El texto traducible va en `span` con `data-es` / `data-en`; la flecha **no** forma parte de esos atributos.
...
- **Variante [`ServiceCard`](./service-card.md):** el shell lleva `group/card`; el texto “Ver servicio” puede ir en un `span` con `data-en`/`data-es`; la flecha es **`span.arrow`** con `<svg>` dentro (no `btn-bounce` en el enlace overlay).
