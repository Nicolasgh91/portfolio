...
| `accordionStyle` | `'native'` \| `'animated'` | No | Default `native`. `native`: `<details>` sin JS. `animated`: botones + panel con **CSS Grid** `grid-template-rows: 0fr` → `1fr`; un solo ítem abierto; script **inline** acotado al elemento con id `faqAnimatedRootId`. |
...
- Script `is:inline` con `define:vars` para el id del root: solo hace `querySelector`/`addEventListener` **dentro** de ese nodo.
